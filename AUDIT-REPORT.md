...
| # | Issue | File | Status |
|---|-------|------|--------|
| S2 | OAuth access token stored in plain memory variable with no expiry tracking | `services/gmailService.ts:91` | Noted — use `expo-secure-store` for persistence |
| S3 | Google OAuth Client ID stored as plain text file on device filesystem | `services/gmailService.ts:45-65` | Noted — move to `expo-secure-store` |
...
| S7 | Race conditions in storage layer (read-modify-write without locking) | `services/storage.ts` throughout | Noted — implement a serialization queue |
...
| # | Issue | File | Status |
|---|-------|------|--------|
| Q1 | Race condition in storage: concurrent read-modify-write can cause data loss | `services/storage.ts` | Noted — same as S7, implement write queue |
...
| Q3 | **`wornDates` and `wornEntries` indices diverge** — `logOutfitWorn` only pushes to `wornEntries` when selfie/note present | `services/storage.ts:377-418` | Noted — always push to `wornEntries` |

### HIGH

| # | Issue | File | Status |
|---|-------|------|--------|
| Q4 | Stale `now` reference in weekly planner — can cause wrong dates after midnight | `app/(tabs)/outfits.tsx:76` | Noted — use `useMemo` or `useRef` to pin date |
...
1. **Storage write queue** (S7/Q1) — Prevents data loss from concurrent writes
2. **Always push to `wornEntries`** (Q3) — Keeps selfie/wear log data consistent
3. **Stale `now` in planner** (Q4) — Pin date reference on mount
4. **Secure token storage** (S2/S3) — Move to `expo-secure-store`
5. **Accessibility** (Q20) — Add labels to all interactive elements
